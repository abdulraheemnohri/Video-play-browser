...
class SettingsRepository(private val dataStore: DataStore<Preferences>) {
    ...
    private val AUTO_PLAY_KEY = stringPreferencesKey("auto_play")
    ...
    private val TRACKING_PROTECTION_KEY = stringPreferencesKey("tracking_protection")
    ...
    suspend fun setHomepage(url: String) {
        dataStore.edit { preferences -> preferences[HOMEPAGE_KEY] = url }
    }
    ...
    suspend fun setAppLanguage(language: String) {
        dataStore.edit { preferences -> preferences[APP_LANGUAGE_KEY] = language }
    }

    suspend fun setSearchEngine(engine: String) {
        dataStore.edit { preferences -> preferences[SEARCH_ENGINE_KEY] = engine }
    }

    suspend fun setBlockPopups(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[BLOCK_POPUPS_KEY] = enabled }
    }

    suspend fun setEnableJavaScript(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[ENABLE_JAVASCRIPT_KEY] = enabled }
    }

    suspend fun setAutoPlay(mode: String) {
        dataStore.edit { preferences -> preferences[AUTO_PLAY_KEY] = mode }
    }

    suspend fun setDefaultPlaybackSpeed(speed: Float) {
        dataStore.edit { preferences -> preferences[DEFAULT_PLAYBACK_SPEED_KEY] = speed }
    }

    suspend fun setRememberPlaybackSpeed(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[REMEMBER_PLAYBACK_SPEED_KEY] = enabled }
    }

    suspend fun setRememberPlaybackPosition(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[REMEMBER_PLAYBACK_POSITION_KEY] = enabled }
    }

    suspend fun setEnablePiP(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[ENABLE_PIP_KEY] = enabled }
    }

    suspend fun setEnableMiniPlayer(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[ENABLE_MINI_PLAYER_KEY] = enabled }
    }

    suspend fun setTrackingProtection(mode: String) {
        dataStore.edit { preferences -> preferences[TRACKING_PROTECTION_KEY] = mode }
    }

    suspend fun setHttpsOnly(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[HTTPS_ONLY_KEY] = enabled }
    }

    suspend fun setClearDataOnExit(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[CLEAR_DATA_ON_EXIT_KEY] = enabled }
    }

    suspend fun setDownloadLocation(path: String) {
        dataStore.edit { preferences -> preferences[DOWNLOAD_LOCATION_KEY] = path }
    }

    suspend fun setWifiOnlyDownloads(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[WI_FI_ONLY_DOWNLOADS_KEY] = enabled }
    }

    suspend fun setAskBeforeDownload(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[ASK_BEFORE_DOWNLOAD_KEY] = enabled }
    }

    suspend fun setReducedMotion(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[REDUCED_MOTION_KEY] = enabled }
    }

    suspend fun setLargeText(enabled: Boolean) {
        dataStore.edit { preferences -> preferences[LARGE_TEXT_KEY] = enabled }
    }

    suspend fun resetVideoSettings() {
        dataStore.edit { prefs ->
            prefs.remove(AUTO_PLAY_KEY)
            prefs.remove(DEFAULT_PLAYBACK_SPEED_KEY)
            prefs.remove(REMEMBER_PLAYBACK_SPEED_KEY)
            prefs.remove(REMEMBER_PLAYBACK_POSITION_KEY)
            prefs.remove(ENABLE_PIP_KEY)
            prefs.remove(ENABLE_MINI_PLAYER_KEY)
        }
    }

    suspend fun resetSitePermissions() {
        // Clear site permissions prefs
    }

    suspend fun resetBrowserSettings() {
        dataStore.edit { prefs ->
            prefs.remove(HOMEPAGE_KEY)
            prefs.remove(SEARCH_ENGINE_KEY)
            prefs.remove(BLOCK_POPUPS_KEY)
            prefs.remove(ENABLE_JAVASCRIPT_KEY)
